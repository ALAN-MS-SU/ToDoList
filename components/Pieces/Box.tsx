...
export function Box({ ID, Name, Status, Update }: Item) {
  if (Status == 0)
    return (
      <View
        className={`flex h-[100px] w-full flex-row items-center justify-around border-b-[2px] border-t-[2px] border-primary`}>
        <Text className="font-bold">{Name}</Text>
        <Button className="h-[50px] w-[75px] bg-green-500 hover:bg-green-700">
          <Check className="h-full w-full text-white" />
        </Button>
        <Button className="h-[50px] w-[75px] bg-red-500 hover:bg-red-700">
